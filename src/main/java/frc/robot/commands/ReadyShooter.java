...
public class ReadyShooter extends SequentialCommandGroup {
	...
	public ReadyShooter(ShooterSubsystem shooter, RotatorSubsystem rotator, IntakeSubsystem intake,
		...
		addCommands(new InstantCommand(() -> intake.setIntakeSpeed(-0.2))
				.andThen(new WaitCommand(0.1))
				.andThen(new InstantCommand(() -> intake.setIntakeSpeed(0))),
				new WaitCommand(0.1),
				new ParallelCommandGroup(
						rotator.rotateToPositionCommand(() -> (distanceToShotValuesMap
								.getInterpolatedRotationAngle(distanceFromGoalSupplier
										.getAsDouble())))
								.withTimeout(1),
						shooter.spinToRps(() -> distanceToShotValuesMap
								.getInterpolatedShooterSpeed(distanceFromGoalSupplier
										.getAsDouble()))));
	}
}
