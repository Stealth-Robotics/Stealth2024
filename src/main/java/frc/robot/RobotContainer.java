...
public class RobotContainer {
    ...
    private final LEDSubsystem ledSubsystem = new LEDSubsystem(rotatorSubsystem::isHomed,
            () -> (rotatorSubsystem.getMotorMode() == NeutralModeValue.Brake), intake::isRingFullyInsideIntake);
    ...
    public RobotContainer() {
        ...
        autoChooser.addOption("Center 4 ring", new CenterAuto(swerveSubsystem, rotatorSubsystem, shooter, intake));
        autoChooser.addOption("center 5 ring", new CenterFiveRing(swerveSubsystem, rotatorSubsystem, shooter, intake));
        ...
        autoChooser.addOption("test",
                new SequentialCommandGroup(new InstantCommand(() -> swerveSubsystem.setInitialPose("testPath")),
        ...
        swerveSubsystem.setDefaultCommand(
                new SwerveDriveTeleop(swerveSubsystem, swerveTranslationYSupplier, swerveTranslationXSupplier,
                        swerveRotationSupplier, swerveRobotOrientedSupplier, swerveSubsystem.isRed()));
        ...
        new Trigger(driverController.leftBumper()).onTrue(new AimAndShootCommand(swerveSubsystem, rotatorSubsystem,
        ...
        new Trigger(driverController.rightBumper()).onTrue(
                new ConditionalCommand(rotatorSubsystem.rotateToPositionCommand(() -> Units.degreesToRotations(53)),
                        new AmpPresetCommand(rotatorSubsystem, shooter, intake), rotatorSubsystem.getAmpOutIntake()));
        ...
        new Trigger(driverController.y())
                .onTrue(new AimAndShootCommand(swerveSubsystem, rotatorSubsystem, shooter, intake, () -> 1.4));
        ...
        new Trigger(driverController.b()).whileTrue(new DriveWhileAlignedToSpeaker(swerveSubsystem,
                swerveTranslationYSupplier, swerveTranslationXSupplier, swerveSubsystem.isRed())
                .alongWith(rotatorSubsystem.rotateWhileDrivingCommand(
                        () -> map.getInterpolatedRotationAngle(swerveSubsystem.getDistanceMetersToGoal()))));

        // Onboard Button Commands

        new Trigger(rotatorHomeButtonSupplier)
                .onTrue(rotatorSubsystem.homeArmCommand().andThen(ledSubsystem.updateDisabledLEDsCommand()));

        new Trigger(rotatorToggleMotorModeButtonSupplier)
                .onTrue(rotatorSubsystem.toggleMotorModeCommand().andThen(ledSubsystem.updateDisabledLEDsCommand()));
